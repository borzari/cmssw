...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  namespace pixelgpudetails {
    ...
    ALPAKA_FN_ACC pixelgpudetails::DetIdGPU getRawId(const SiPixelROCsStatusAndMapping *cablingMap,
                                                     uint8_t fed,
                                                     uint32_t link,
                                                     uint32_t roc) {
    ...
    class fillHitsModuleStart {
      ...
      ALPAKA_FN_ACC void operator()(const TAcc &acc,
        ...
        ALPAKA_ASSERT_OFFLOAD(nMaxModules < 4096);  // easy to extend at least till 32*1024
        ALPAKA_ASSERT_OFFLOAD(nMaxModules > 1024);

        ALPAKA_ASSERT_OFFLOAD(1 == gridDim.x);
        ALPAKA_ASSERT_OFFLOAD(0 == blockIdx.x);
        ...
        ALPAKA_ASSERT_OFFLOAD(moduleStart[maxH] >= moduleStart[maxH - 1]);
        ALPAKA_ASSERT_OFFLOAD(moduleStart[maxH + 1] >= moduleStart[maxH]);
        ALPAKA_ASSERT_OFFLOAD(moduleStart[nMaxModules] >= moduleStart[maxH + 1]);
